...
BUSINESS_MAILCHIMP_EMAIL_LIST_ID = getattr(settings, "BUSINESS_MAILCHIMP_EMAIL_LIST_ID", None)

if BUSINESS_MAILCHIMP_EMAIL_LIST_ID is None:
    raise NotImplementedError("BUSINESS_MAILCHIMP_EMAIL_LIST_ID must be set in the settings, something like us17")
...
class Mailchimp(object):
    def __init__(self, is_business):
        self.key = MAILCHIMP_API_KEY
        self.is_business = is_business
        self.api_url = 'https://{dc}.api.mailchimp.com/3.0'.format(dc=MAILCHIMP_DATA_CENTER)
        self.list_id = BUSINESS_MAILCHIMP_EMAIL_LIST_ID if self.is_business else MAILCHIMP_EMAIL_LIST_ID
